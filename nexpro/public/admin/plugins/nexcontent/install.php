...
function show_prerequisites() {
    global $CONF_SE, $_CONF, $_TABLES, $pi_name, $pi_display_name, $pi_version, $gl_version, $pi_url,$CONF_SE_PREREQUISITE;
    $disabled = '';
    ...
    if ($image == 'icon_fail.png') {
        $disabled = ' disabled="disabled"';
        COM_errorLog("Cannot install $pi_name: Cannot write to \"{$CONF_SE_PREREQUISITE['uploadpath']}\"");
    }
    $p->set_var('prereqs', '<li style="list-style-image: url('.$_CONF['layout_url'].'/'.$pi_name.'/images/admin/'.$image.');"> Write permissions on "'.$CONF_SE_PREREQUISITE['uploadpath'] . '"', true);
