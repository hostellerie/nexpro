...
$CONF_FORUM['use_nexmenu'] = true;          // Should nexmenu be used for this menublock;
...
/*****************************************************************************************************
 * When posting a new topic, there are extra notification options
 * These settings control these option defaults
 ****************************************************************************************************/

// The extra notification options can be restricted to only appear for a selected group or all users
$CONF_FORUM['notifyoptions_allowedGroup']  = 'Logged-in Users';   // Define the group name to enable the extra notification feature
$CONF_FORUM['notify_defaultOption']        = 'custom';            // Set the default - valid Options are basic,full,summary or custom
...
*/
$CONF_FORUM['notifyall_allowedGroup'] = 'Root';     //  Group to allow the notify all members feature
$CONF_FORUM['notifyall_default']      = 0;          // Set to 1 if you want this option enabled by default
...
// Determine what File Management Plugin to use
if (DB_getItem($_TABLES['plugins'],'pi_enabled',"pi_name='nexfile'") == 1) {
    $CONF_FORUM['FM_PLUGIN']        = 'nexfile';
} elseif (DB_getItem($_TABLES['plugins'],'pi_enabled',"pi_name='filemgmt'") == 1) {
    $CONF_FORUM['FM_PLUGIN']        = 'filemgmt';
} else {
    $CONF_FORUM['FM_PLUGIN']        = '';
}
