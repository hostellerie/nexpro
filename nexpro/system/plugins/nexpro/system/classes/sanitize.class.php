...
/*  Identified issues that would be nice to address

- Need to call initFilter if I need to clean another variable and existing clean data exists then I get an array back
  Maybe adding an option to public getDbData(), getCleanData(), getWebData() to init before returning
- Have a method to just format data for DB or WEB which is already filtered instead of requiring that extra overhead again
...
class sanitizer {
    ...
    public function setCheckhtml($state) {
        if ($state === true or $state == 'on') {
            $this->_checkhtml = true;
        } elseif ($state === false or $state == 'off') {
            $this->_checkhtml = false;
        }
    }
    ...
    private function _santizeData($type='',$data='') {
        ...
        if (!empty($data)) {
            ...
            if (!is_array($this->_dirtydata) OR count($this->_dirtydata) == 0) {
                if ($type == 'int') {
                    return 0;
                } else {
                    return FALSE;
    ...
    /* Return a single array containing each var type in a single dimension array */
    function normalize($data='') {
        $retval = array();
        if (!is_array($data) OR empty($data)) {
            $data = $this->_cleandata;
        }
        foreach ($this->_filtermodes as $type) {
            if (count($data[$type]) > 0) {
                foreach($data[$type] as $var => $value) {
                    $retval[$var] = $value;
                }
            }
        }
    ...
    */
    public function getCleanData($type='',$data='',$initfilter=true) {

        //if (empty($data) OR is_array($data) AND count

        $retval = $this->_santizeData($type,$data);
        $this->_makeGlobal();

        // By Default: reset the filter class data now that we have processed the filtering
        if ($initfilter) $this->initFilter();

        return $retval;
    }

    public function getWebData($type='',$data='',$initfilter=true) {
    ...
    public function getDbData($type='',$data='',$initfilter=true) {
