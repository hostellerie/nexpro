 ...
 *                    Kristian Koehntopp
 *
 * $Id: template.class.php,v 1.7 2009/08/14 00:44:30 blaine.lang Exp $
 *
 */
...
class Template
{
 ...
 /**
  * If trace mode is enabled then the parsed HTML will include comments before and after each template
  * Used to allow you to view the page HTML and see the template used
  * Can be enabled by changing this value to true or Setting $_TRACE_TEMPLATES in the themes functions.php to true
  *
  * @var       int
  * @access    private
  * @see       set_root
  ...
  */
  function Template($root = '.', $unknowns = 'remove') {
    global $_TRACE_TEMPLATES;
    if ($this->debug & 4) {
      echo "<p><b>Template:</b> root = $root, unknowns = $unknowns</p>\n";
    }
    if(isset($_TRACE_TEMPLATES) AND $_TRACE_TEMPLATES) {
        $this->trace = true;
    }
    $this->set_root($root);
  ...
  */
  function parse($target, $varname, $append = false) {
    if (!is_array($varname)) {
      ...
      $str = '';
      if ($this->trace) {
        $str = '<!-- Start Template: ' . $this->file[$varname] .' -->' . LB;
      }
      $str .= $this->subst($varname);
      if ($this->trace) {
        $str .= '<!-- End of Template: ' . $this->file[$varname] .' -->' . LB;
      }
      if ($append) {
