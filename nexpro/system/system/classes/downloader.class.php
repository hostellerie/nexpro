...
class downloader
{
    ...
    */
    ...
    /**
    * Add to the array of availble Mime Types and Extensions.
    * Usefull when you want to add to the default defined in this class
    *
    * @param    array       $newfiletype  Array of new file types -
    *                       same array structure as defined in $this->_setAvailableExtensions
    *
    */
    function addAvailableExtensions($newfiletypes = array())
    {
        // Add a record(s) to the _availableExtensions and  _availableMimetypes.
        foreach ($newfiletypes as $ext => $mime) {
            if (!in_array ($ext, $this->_availableExtensions)) {
                array_push($this->_availableExtensions,$ext);
                $this->_availableMimeTypes = array_merge($this->_availableMimeTypes,array($ext => $mime));
            }
        }
    }
    ...
    */
    function downloadFile($fileName)
    {
        ...
        if ($this->checkExtension($fextension)) {
            ...
            /* July 29/2009 (Blaine Lang)
             * Added additional code to clear buffer, session and set Cache Control header tags
             * Required to solve an IE6 issue with downloads over SSL
            */
